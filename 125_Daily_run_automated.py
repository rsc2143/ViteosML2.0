...
m_before_function_definitions = memory_profiler.memory_usage()
...
#Change added on 11-01-2021 as per Rohit. This code is to make umb_ob table along with smb_ob table as well.
...
m_after_function_definitions = memory_profiler.memory_usage()
mem_diff_after_function_definitions = m_after_function_definitions[0] - m_before_function_definitions[0]
print(f"It took {mem_diff_after_function_definitions} Mb to execute this method")
#It took 0.1484375 Mb to execute this method
client = 'Weiss'
...
#meo_df = pd.read_csv(filepaths_meo_df)
#
...
if(closed_df_interacting.shape[0] != 0):
...
#thoda time-taking - below if and loop statement
...
start_time_updown = timeit.default_timer()
...
#    Change added on 06-02-2021 as per Rohit. This code is faster as it is vectorized. The following line was commented out : df_213_1[['map_match','amt_match','fund_match','curr_match','sd_match']] = df_213_1.apply(lambda row : amountelim(row), axis = 1,result_type="expand")
...
#thoda time-taking - below 2 lines
...
#thoda time-taking - below 1 lines
...
filepaths_X_test = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\X_Test_for_Pratik_setup_' + setup_code + '_date_' + str(date_i) + '_time_experiment_04_12_2020.csv'
...
filepaths_final_umr_table_copy_new_to_write = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\final_umr_table_copy_new_to_write_setup_' + setup_code + '_date_' + str(date_i) + '.csv'
...
if(comment_table_eq_swap_copy.shape[0] != 0):
	...
	comment_table_eq_swap_copy = pd.DataFrame()
...
filepaths_comment_table_eq_swap_copy_to_write = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\comment_table_eq_swap_copy_to_write_setup_' + setup_code + '_date_' + str(date_i) + '.csv'
...
#        final_mtm_table_copy_new['probability_UMT'].iloc[i] = float(decimal.Decimal(random.randrange(17, 100))/1000)
    ...
    filepaths_mtm_df_ex_and_fx_copy_new_to_write = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\mtm_df_ex_and_fx_copy_new_setup_' + setup_code + '_date_' + str(date_i) + '.csv'
...
#    final_oto_umb_table_copy_new.to_csv(filepaths_final_oto_umb_table_copy_new)
    ...
    filepaths_final_oto_umb_table_copy_new_to_write = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\final_oto_umb_table_copy_new_to_write_setup_' + setup_code + '_date_' + str(date_i) + '.csv'
...
if(closed_df.shape[0] != 0):
    ...
    filepaths_final_closed_df = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\final_closed_df_setup_' + setup_code + '_date_' + str(date_i) + '_3.csv'
...
umb_carry_forward_columns_to_select_from_meo_df = ['ViewData.BreakID', \
                                                   ...
                                                   'ViewData.Task ID'
#                                                  Change made on 29-01-2021 as per Rohit. This change will include the comment given to UMB carry forward directly to final predicitons as well
...
if(umb_carry_forward_df.shape[0] != 0):
    ...
    change_names_of_umb_carry_forward_df_mapping_dict = {
                                                ...
                                                'ViewData.Task Business Date' : 'BusinessDate',
                                                'ViewData.Source Combination Code' : 'SourceCombinationCode',
                                                'ViewData.InternalComment2' : 'PredictedComment'
                                            }
...
def umb_comment(bk0,bk1):
    ...
    if tt.lower() == 'dividend':
        ...
        comment = 'Difference in fee and commision booked between' + ' ' + str(pb) + ' & Geneva on SD ' + str(sd)
    elif tt.lower() == 'sell':
...
def ui_action_column_test_dont_use(param_final_df):
    param_final_df.loc[((param_final_df['ML_flag'] == 'Not_Covered_by_ML')),'ActionType'] = 'No Prediction'    
    param_final_df.loc[((param_final_df['Predicted_Status'].isin(['OB','SMB'])) & (param_final_df['PredictedComment'] == '') & (param_final_df['ML_flag'] == 'ML')),'ActionType'] = 'No Action'
    param_final_df.loc[((param_final_df['Predicted_Status'].isin(['OB','SMB'])) & (param_final_df['PredictedComment'] != '') & (param_final_df['ML_flag'] == 'ML')),'ActionType'] = 'COMMENT'
    param_final_df.loc[((param_final_df['Predicted_Status'] == 'UCB') & (param_final_df['PredictedComment'] == '') & (param_final_df['ML_flag'] == 'ML')),'ActionType'] = 'CLOSE'
    param_final_df.loc[((param_final_df['Predicted_Status'].isin(['UMB','UMR','UMT'])) & (param_final_df['PredictedComment'] == '') & (param_final_df['ML_flag'] == 'ML')),'ActionType'] = 'PAIR'
#    param_final_df.loc[((param_final_df['Predicted_Status'].isin(['SMB','UMB','UMR','UMT'])) & (param_final_df['PredictedComment'] != '')),'ActionType'] = 'PAIR WITH COMMENT'
    param_final_df.loc[((param_final_df['Predicted_Status'].isin(['UMB','UMR','UMT'])) & (param_final_df['PredictedComment'] != '')),'ActionType'] = 'PAIR WITH COMMENT'
    param_final_df.loc[(param_final_df['ActionType'].isnull()),'ActionType'] = 'Status not covered'
    param_final_df.loc[(param_final_df['ActionType'].isna()),'ActionType'] = 'Status not covered'
    param_final_df['ActionType'] = param_final_df['ActionType'].astype(str)
    ...
    param_final_df.loc[((param_final_df['Predicted_Status'] == 'UCB') & (param_final_df['PredictedComment'] == '') & (param_final_df['ML_flag'] == 'ML')),'ActionTypeCode'] = 2
    param_final_df.loc[((param_final_df['Predicted_Status'].isin(['UMB','UMR','UMT'])) & (param_final_df['PredictedComment'] == '') & (param_final_df['ML_flag'] == 'ML')),'ActionTypeCode'] = 1
#    param_final_df.loc[((param_final_df['Predicted_Status'].isin(['SMB','UMB','UMR','UMT'])) & (param_final_df['PredictedComment'] != '')),'ActionTypeCode'] = 4
    param_final_df.loc[((param_final_df['Predicted_Status'].isin(['UMB','UMR','UMT'])) & (param_final_df['PredictedComment'] != '')),'ActionTypeCode'] = 4
    param_final_df.loc[(param_final_df['ActionTypeCode'].isnull()),'ActionTypeCode'] = 0
    param_final_df.loc[(param_final_df['ActionTypeCode'].isna()),'ActionTypeCode'] = 0
    param_final_df['ActionTypeCode'] = param_final_df['ActionTypeCode'].astype(int)
...
filepaths_final_df_2_before_making_umt_from_umr = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\final_df_2_before_making_umt_from_umr_setup_' + setup_code + '_date_' + str(date_i) + '_rerun_for_business.csv'
...
filepaths_final_df_2_after_making_umt_from_umr = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\final_df_2_after_making_umt_from_umr_setup_' + setup_code + '_date_' + str(date_i) + '_2.csv'
...
filepaths_final_df_2 = '\\\\vitblrdevcons01\\Raman  Strategy ML 2.0\\All_Data\\' + client + '\\final_df_2_setup_' + setup_code + '_date_' + str(date_i) + '_rerun_for_business3.csv'
...
#Read from 137, ReconDB instead of ReconDB_ML
